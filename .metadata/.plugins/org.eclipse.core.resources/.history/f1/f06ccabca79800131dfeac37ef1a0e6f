 ...
 *
 */
public class MonoalphabeticCpaNpaAnalysis implements
de.tudarmstadt.gdi1.project.analysis.monoalphabetic.MonoalphabeticCpaNpaCryptanalysis {
	...
	/**
	 * Ermittelt aus einem Klartext und dem jeweiligen Chiffrétext den benutzten 
	 * Schlüssel. Falls aus mangelnder Länge nicht der gesamte Schlüssel ermittelbar ist, 
	 * werden die unbekannten Stellen mit Leerzeichen ausgefüllt.
	 * @param ciphertext der verschlüsselte Text
	 * @param plaintext der Klartext
	 * @param alphabet das benutzte Alphabet
	 * @return der ermittelte Schlüssel (evtl. nicht vollständig)
	 */
	@Override
	public char[] knownPlaintextAttack(String ciphertext, String plaintext,
		...
		try {
			//falls Klar- und Chiffrétext nicht die gleiche Länge besitzen, wird eine 
			//Exception geworfen
			if (cipherArray.length != plainArray.length) throw new InvalidTextException();
		}
		catch(InvalidTextException ie) {
			System.out.println("Die eingegebenen Texte besitzen nicht die gleiche Länge!");
		}
