...
Utils {
	...
	public char[] asCharArray() {
		...
		/**
		 * Erzeugt einen Iterator für das Alphabet.
		 * @return Iterator
		 */
		@Override
		...
		public de.tudarmstadt.gdi1.project.alphabet.Alphabet shiftAlphabet(
			...
			for(int i = 0; i < alphabet.size(); i++){  //Iteriert durch das Alphabet und verschiebt es um den wert von key
				if(i+shift > alphabet.size()-1) // wenn Zähler + Verschiebung größer als die Länge des Alphabets ist 
		...
		/**
		 * Erzeugt ein zufällig angeordnetes Alphabet.
		...
		public de.tudarmstadt.gdi1.project.alphabet.Alphabet randomizeAlphabet(
			...
			int count = 0; //Counter
			Set<Character> tempSet = new HashSet<>(); //temporäres Hilfsset
			...
			while (count < alphabet.asCharArray().length) {
				...
				rAlphabet[count] = getChar(actualRandom); //setzt ansonsten gegenwärtige
				...
				//Indices hinzugefügt
				tempSet.add(rAlphabet[count]); //dem temporären Set wird der gegenwärtige
				//Buchstabe des neuen Alphabets hinzugefügt
				count++; //der Counter wird hochgezählt
			}
		...
		/**
		 * Fügt dem Alphabet einen Character hinzu, sofern möglich.
		 * @param chr der hinzuzufügende Character
		 */
